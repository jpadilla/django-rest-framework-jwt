...
class TestCustomResponsePayload(BaseTestCase):
    def setUp(self):
        ...
        views.jwt_response_payload_handler = test_utils.jwt_response_payload_handler
...
class ObtainJSONWebTokenTests(BaseTestCase):
    ...
    def test_jwt_login_with_expired_token(self):
        ...
        client = APIClient(enforce_csrf_checks=True)
        response = client.post(
            "/auth-token/", self.data, HTTP_AUTHORIZATION=auth, format="json"
        )
...
class VerifyJSONWebTokenTestsSymmetric(TokenTestCase):
    def test_verify_jwt(self):
        ...
        response = client.post(
            "/auth-token-verify/", {"token": orig_token}, format="json"
        )
    ...
    def test_verify_jwt_fails_with_bad_token(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertRegexpMatches(
            response.data["non_field_errors"][0], "Error decoding signature"
        )
    ...
    def test_verify_jwt_fails_with_missing_user(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertRegexpMatches(
            response.data["non_field_errors"][0], "User doesn't exist"
        )
...
class RefreshJSONWebTokenTests(TokenTestCase):
    ...
    def test_refresh_jwt(self):
        ...
        # Now try to get a refreshed token
        response = client.post(
            "/auth-token-refresh/", {"token": orig_token}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_refresh_jwt_after_refresh_expiration(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["non_field_errors"][0], "Refresh has expired.")

    def test_refresh_jwt_with_cookies(self):
        """
        Test getting a refreshed token from original token works

        No date/time modifications are neccessary because it is assumed
        that this operation will take less than 300 seconds.
        The token is passed through cookies rather than POST data.
        """
        api_settings.JWT_AUTH_COOKIE = "plop"
        client = APIClient(enforce_csrf_checks=True)
        # This way the client will have its token stored in a cookie
        response = client.post("/auth-token/", self.data, format="json")

        orig_token = response["token"]
        orig_token_decoded = utils.jwt_decode_handler(orig_token)
        ...
        # Now try to get a refreshed token
        response = client.post("/auth-token-refresh/", {}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def tearDown(self):
        ...
        api_settings.JWT_AUTH_COOKIE = DEFAULTS["JWT_AUTH_COOKIE"]
